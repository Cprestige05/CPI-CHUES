...
import {
  ...
  Phone, Bell, CheckCircle, FileText, MessageSquare, Download,
  ArrowUpRight, TrendingUp, ChevronRight, Sparkles,
  MapPin, User, Calculator, CircleDot,
} from 'lucide-react';
...
import {
  ...
  InlineAlert, ProgressBar, useCountUp, Btn, DS,
} from './ui';
...
export default function ClientDashboardHome({ user }: Props) {
  ...
  const alerteBadge = 'none' as 'none' | 'warning' | 'danger';
    ...
    <div style={{ fontFamily: 'var(--font-sans)', color: 'var(--foreground)', width: '100%', maxWidth: '100%', margin: 0, padding: '0 0 64px' }}>
        ...
        <div style={{
          ...
          {[
            ...
            <div key={s.label} style={{
              ...
              <div style={{ fontFamily: 'var(--font-sans)', fontSize: '0.5625rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.35)', marginBottom: 3 }}>{s.label}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(168px, 1fr))', gap: 12, marginBottom: 24 }}>
        ...
        <KPICard icon={<AlertTriangle size={19} />} label="Alertes"
          value="Aucune alerte" sub="Statut en temps réel"
          ...
          <div style={{ padding: '20px 22px' }}>
            {ACTIVITIES.length === 0 ? (
              ...
              <TimelineItem key={i}
                ...
                title={item.text} date={item.date} time={item.hour}
                last={i === ACTIVITIES.length - 1} delay={200 + i * 80} />
            ))}
          </div>
          <CardDivider />
