...
import {
  ...
  Zap, Database, HardDrive, Key, Smartphone,
} from 'lucide-react';
...
import AdminDemandesEnrolement from './AdminDemandesEnrolement';
    ...
    <div className="space-y-6" style={{ width: '100%' }}>
      ...
      {section === 'overview' && (
        <>
          ...
          {/* Raccourcis rapides — accès direct aux espaces de gestion */}
          <div className={card} style={cardStyle}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: A.text, marginBottom: 16 }}>Raccourcis rapides</h3>
          ...
          {/* Dernières demandes — aperçu, lien vers la vue complète */}
          <div className={card} style={cardStyle}>
            <div className="flex items-center justify-between mb-4">
              <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: A.text }}>Dernières demandes</h3>
              <button onClick={() => navigate('demandes')} className="flex items-center gap-1"
                style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '0.8125rem', fontWeight: 700, color: A.bordeaux }}>
                Voir tout <ChevronRight className="w-3.5 h-3.5" />
              </button>
            </div>
            {rows.length === 0 ? (
              <div style={{ fontSize: '0.8125rem', color: A.muted }}>Aucune demande pour le moment.</div>
            ) : (
              <div className="space-y-2">
                {rows.slice(0, 5).map(r => {
                  const label = TIMELINE_STEPS[r.activeStep]?.label ?? '—';
                  return (
                    <button key={r.c.id} onClick={() => navigate('demandes')} className="w-full flex items-center gap-3 p-3 text-left"
                      style={{ background: '#FAF7F7', border: `1px solid ${A.border}`, borderRadius: 'var(--r-sm)', cursor: 'pointer' }}
                      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = '#F5ECEE'; }}
                      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = '#FAF7F7'; }}>
                      <div className="w-9 h-9 flex items-center justify-center flex-shrink-0" style={{ background: `${A.bordeaux}12`, borderRadius: 'var(--r-sm)' }}>
                        <FileText className="w-4 h-4" style={{ color: A.bordeaux }} />
                      </div>
                      <div className="flex-1" style={{ minWidth: 120 }}>
                        <div style={{ fontSize: '0.8125rem', fontWeight: 700, color: A.text }}>{r.c.name}</div>
                        <div style={{ fontSize: '0.6875rem', color: A.muted }}>{r.c.ref} · {r.c.projectNom}</div>
                      </div>
                      <span style={{ fontSize: '0.6875rem', fontWeight: 700, color: A.bordeaux, background: `${A.bordeaux}10`, padding: '4px 10px', borderRadius: 'var(--r-full)', flexShrink: 0 }}>{label}</span>
                    </button>
                  );
                })}
              </div>
          ...
          {/* Analyse du portefeuille — santé du pipeline puis statut des documents */}
          <div>
            <div style={{ color: A.gold, fontSize: '0.6875rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>Analyse du portefeuille</div>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.125rem', fontWeight: 800, color: A.text }}>Santé du pipeline &amp; statut des documents</h3>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            {/* 3. Pièces justificatives par statut */}
            <div className={card} style={cardStyle}>
      ...
      {section === 'demandes' && <AdminDemandesEnrolement />}
...
function SystemeView() {
  ...
  const agentsCount = BUILTIN_STAFF.filter(s => s.role === 'commercial').length + staff.filter(s => s.role === 'commercial').length;
...
const ROLE_CFG: Record<string, { label: string; color: string }> = {
  user:        { label: 'Client',         color: A.bordeaux },
  'commercial': { label: 'Agent CPI',      color: A.gold },
...
function UsersView({ agentName }: { agentName: string }) {
  ...
  const [form, setForm] = useState<{ role: 'user' | 'commercial' | 'admin'; name: string; email: string; phone: string }>({ role: 'user', name: '', email: '', phone: '' });
  ...
  const nbAgents = allStaff.filter(s => s.role === 'commercial').length;
  ...
  const addUser = () => {
    ...
    setShowAdd(false);
    setForm({ role: 'user', name: '', email: '', phone: '' });
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="flex gap-2 mt-1 flex-wrap">
                  {(['user', 'commercial', 'admin'] as const).map(rr => (
